...
export default function CreationsPage() {
  ...
  if (status === 'loading') return <div className="min-h-screen bg-zinc-950 flex items-center justify-center text-zinc-500"><Loader2 className="w-5 h-5 animate-spin" /></div>
  if (status !== 'authenticated') return (
    <div className="min-h-screen bg-zinc-950 text-white flex items-center justify-center text-center px-6">
      <div>
  ...
  return (
    <main className="min-h-screen bg-gradient-to-b from-zinc-950 to-zinc-900 text-white">
      <div className="max-w-5xl mx-auto px-4 py-6" style={{ paddingTop: 'calc(env(safe-area-inset-top,0px) + 16px)' }}>
        <Link href="/workspace" className="inline-flex items-center gap-1.5 text-sm text-zinc-500 hover:text-white mb-4">
          <ArrowLeft className="w-4 h-4" /> Builder
        </Link>
        <h1 className="text-2xl sm:text-3xl font-bold tracking-tight mb-1">Your creations</h1>
        <p className="text-sm text-zinc-500 mb-4">Every site, video, and image you've made — in one place.</p>
          ...
          {FILTERS.map((f) => (
            <button
              ...
              className={cn(
                ...
                filter === f.k ? 'bg-white text-zinc-900 border-white' : 'bg-white/5 text-zinc-300 border-white/10 hover:bg-white/10',
              )}
            >
              {f.label} <span className={cn('ml-1 text-xs', filter === f.k ? 'text-zinc-500' : 'text-zinc-600')}>{counts[f.k] ?? 0}</span>
            </button>
          ))}
        </div>
        ...
        {!items ? (
          <div className="py-20 flex justify-center text-zinc-600"><Loader2 className="w-5 h-5 animate-spin" /></div>
        ) : shown.length === 0 ? (
          <div className="py-16 text-center rounded-2xl border border-dashed border-white/10">
            ...
            <p className="text-zinc-300 mb-1">Nothing here yet</p>
            <p className="text-sm text-zinc-500 mb-4">Build a site, make a video, or design a logo — they'll all land here.</p>
              ...
              const Inner = (
                <>
                  ...
                  <div className="p-2.5">
                    ...
                    {it.date ? <p className="text-[10px] text-zinc-600 mt-0.5">{new Date(it.date).toLocaleDateString()}</p> : null}
                  </div>
                </>
              )
              const cls = 'group rounded-xl border border-white/10 bg-white/[0.02] overflow-hidden hover:border-violet-500/40 transition text-left'
              return it.kind === 'site'
      ...
      {lightbox && (
        <div className="fixed inset-0 z-[120] bg-black/85 backdrop-blur-md flex flex-col items-center justify-center p-5" onClick={() => setLightbox(null)}>
          ...
          <div className="w-full max-w-lg" onClick={(e) => e.stopPropagation()}>
            {lightbox.kind === 'video'
              ? <video src={lightbox.mediaUrl} controls autoPlay loop className="w-full rounded-2xl border border-white/10 bg-black" />
              // eslint-disable-next-line @next/next/no-img-element
