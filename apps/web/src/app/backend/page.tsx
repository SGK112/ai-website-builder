  ...
  return (
    <Shell>
      <div className="mb-6 flex items-baseline justify-between">
        <div>
          <h1 className="text-xl font-semibold text-foreground">Data Studio</h1>
          <p className="text-sm text-muted-foreground">{overview.name} · <code className="text-muted-foreground">{overview.baseUrl}</code></p>
        </div>
        <button onClick={() => { loadOverview(); if (active) loadRows(active) }} className="rounded-lg bg-muted px-3 py-1.5 text-xs text-foreground/80 hover:bg-muted/70">Refresh</button>
      </div>

      <div className="grid grid-cols-[200px_1fr] gap-6">
        {/* Sidebar — collections + users */}
        <aside className="space-y-1">
          <div className="mb-1 text-[10px] font-medium uppercase tracking-wider text-muted-foreground">Collections</div>
          {overview.collections.length === 0 && <p className="text-xs text-muted-foreground">No data yet. Your app writes rows here.</p>}
          {overview.collections.map((c) => (
            <button key={c.collection} onClick={() => setActive(c.collection)}
              className={`flex w-full items-center justify-between rounded-lg px-3 py-2 text-left text-sm ${active === c.collection ? 'bg-violet-600/20 text-violet-200' : 'text-foreground/80 hover:bg-muted'}`}>
              <span className="truncate">{c.collection}</span>
              <span className="ml-2 shrink-0 rounded bg-muted px-1.5 text-[10px] text-muted-foreground">{c.count}</span>
            </button>
          ))}
          <div className="flex items-center justify-between rounded-lg px-3 py-2 text-sm text-muted-foreground">
            <span>Users (auth)</span>
            <span className="rounded bg-muted px-1.5 text-[10px]">{overview.userCount}</span>
          </div>
        </aside>
        ...
        <main className="space-y-8">
          <section>
            <h2 className="mb-2 text-sm font-medium text-foreground">{active || 'Select a collection'}</h2>
            {loadingRows ? <p className="text-xs text-muted-foreground">Loading rows…</p> : (
              <div className="overflow-x-auto rounded-lg border border-border">
                <table className="w-full text-left text-xs">
                  ...
                  <tbody>
                    {rows.length === 0 && <tr><td colSpan={columns.length + 2} className="px-3 py-6 text-center text-muted-foreground">No rows.</td></tr>}
                    {rows.map((row) => (
          ...
          <section>
            <h2 className="mb-2 text-sm font-medium text-foreground">Integrations</h2>
...
function RowView({ row, columns, onDelete, onSave }: { row: Row; columns: string[]; onDelete: () => void; onSave: (json: string) => void }) {
  ...
  return (
    <>
      <tr className="border-t border-border text-foreground/80">
      ...
      {editing && (
        <tr className="border-t border-border bg-black/30">
          <td colSpan={columns.length + 2} className="px-3 py-2">
            <textarea value={draft} onChange={(e) => setDraft(e.target.value)} rows={Math.min(12, draft.split('\n').length + 1)}
              className="w-full rounded bg-black/40 p-2 font-mono text-[11px] text-foreground outline-none ring-1 ring-white/10" />
            <div className="mt-1 flex gap-2">
              ...
              <button onClick={() => setEditing(false)} className="rounded bg-muted px-2 py-1 text-[11px] text-foreground/80">Cancel</button>
            </div>
          </td>
        </tr>
      )}
...
function AddRowForm({ defaultCollection, qs, onAdded }: { defaultCollection: string; qs: () => string; onAdded: (collection: string) => void }) {
  ...
  if (!open) return (
    <button onClick={start} className="mt-2 rounded-lg bg-muted px-3 py-1.5 text-xs text-foreground/80 hover:bg-muted/70">+ Add row</button>
  )
  return (
    <div className="mt-2 rounded-lg border border-border bg-black/30 p-3">
      <div className="mb-2 flex items-center gap-2">
        ...
        <input value={collection} onChange={(e) => setCollection(e.target.value)} placeholder="products"
          className="w-40 rounded bg-muted/50 px-2 py-1 font-mono text-[11px] text-foreground outline-none ring-1 ring-white/10 placeholder-zinc-600" />
        <span className="text-[10px] text-muted-foreground">Use “products” for a trusted store price catalog.</span>
      </div>
      ...
      <div className="mt-1 flex items-center gap-2">
        ...
        <button onClick={() => setOpen(false)} className="rounded bg-muted px-2 py-1 text-[11px] text-foreground/80">Cancel</button>
...
function SecretsSection({ overview, qs, reload }: { overview: Overview; qs: () => string; reload: () => void }) {
  ...
  return (
    <section>
      <h2 className="mb-1 text-sm font-medium text-foreground">Secrets</h2>
      <p className="mb-3 text-[11px] text-muted-foreground">Server-side only. Use these in backend actions (e.g. <code>STRIPE_SECRET_KEY</code>) — never exposed to the browser.</p>
      <div className="space-y-1.5">
        {overview.secrets.map((s) => (
          <div key={s.key} className="flex items-center justify-between rounded-lg border border-border bg-muted/40 px-3 py-2 text-xs">
            <span className="font-mono text-foreground/80">{s.key}</span>
            <span className="flex items-center gap-3">
              <span className="font-mono text-muted-foreground">{s.masked}</span>
              <button onClick={() => remove(s.key)} className="text-muted-foreground hover:text-red-400">remove</button>
            </span>
          </div>
        ))}
        {overview.secrets.length === 0 && <p className="text-[11px] text-muted-foreground">No secrets yet.</p>}
      </div>
      <div className="mt-3 flex gap-2">
        <input value={key} onChange={(e) => setKey(e.target.value.toUpperCase())} placeholder="STRIPE_SECRET_KEY"
          className="w-48 rounded-lg bg-muted/50 px-3 py-1.5 font-mono text-xs text-foreground outline-none ring-1 ring-white/10 placeholder-zinc-600" />
        <input value={value} onChange={(e) => setValue(e.target.value)} placeholder="value" type="password"
          className="flex-1 rounded-lg bg-muted/50 px-3 py-1.5 text-xs text-foreground outline-none ring-1 ring-white/10 placeholder-zinc-600" />
...
function ActionsSection({ appId, qs }: { appId: string; qs: () => string }) {
  ...
  return (
    <section>
      <h2 className="mb-1 text-sm font-medium text-foreground">Server actions</h2>
      ...
      <div className="space-y-1.5">
        ...
        {actions.length === 0 && <p className="text-[11px] text-muted-foreground">No actions yet.</p>}
      </div>
      <div className="mt-3 grid grid-cols-[110px_90px_1fr] gap-2">
        <input value={name} onChange={(e) => setName(e.target.value)} placeholder="send-email" className="rounded-lg bg-muted/50 px-3 py-1.5 font-mono text-xs text-foreground outline-none ring-1 ring-white/10 placeholder-zinc-600" />
        <select value={method} onChange={(e) => setMethod(e.target.value)} className="rounded-lg bg-muted/50 px-2 py-1.5 text-xs text-foreground outline-none ring-1 ring-white/10">
          {['POST', 'GET', 'PUT', 'PATCH', 'DELETE'].map((m) => <option key={m} className="bg-card">{m}</option>)}
        </select>
        <input value={url} onChange={(e) => setUrl(e.target.value)} placeholder="https://api.openai.com/v1/chat/completions" className="rounded-lg bg-muted/50 px-3 py-1.5 font-mono text-xs text-foreground outline-none ring-1 ring-white/10 placeholder-zinc-600" />
      </div>
      <textarea value={headersText} onChange={(e) => setHeadersText(e.target.value)} rows={3} className="mt-2 w-full rounded-lg bg-muted/50 p-2 font-mono text-[11px] text-foreground outline-none ring-1 ring-white/10" />
...
function Shell({ children }: { children: React.ReactNode }) {
  return (
    <div className="min-h-screen bg-[#0b0b0f] px-6 py-8 text-foreground">
