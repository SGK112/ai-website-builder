...
${html}
  ...
  // collect the user's real details before generating.
  ...
  // Mobile app starter (TemplatesPanel) — resets to a fresh Expo project and
  ...
  const startMobileTemplate = async (starterPrompt: string) => {
    ...
    setBuildTarget('expo')
    setChatMessages(prev => [...prev, { role: 'user', content: starterPrompt }])
    await handleGenerateMultiTarget('expo', starterPrompt)
  }
...
`)
  ...
  // AI image generation (ImagesPanel "Generate" button). Reads the current
  // prompt/style/aspect state and stows the result in generatedImageUrl.
  ...
  // Drag a panel image over the preview — flag drag state + tell the iframe so
  // it can light up drop targets.
  const onImageDragStart = (url: string) => {
    setDraggedImageUrl(url)
  ...
  // --- Video panel handlers (VideoPanel owns presentation; these own state) ---
  // Upload a still to animate (image-to-video via SVD).
  const uploadVideoSource = async (file: File) => {
  ...
  // Text-to-video, or image-to-video when a source image is set.
  const generateVideo = async () => {
    ...
    try {
      const response = await fetch('/api/ai/video', {
        ...
        body: JSON.stringify(
          isImageMode
            ? { action: 'image-to-video', imageUrl: videoSourceImage, prompt: videoPrompt || undefined, model: 'svd' }
  ...
  // Drop the generated video into the page HTML before </body>.
  const insertVideoIntoWebsite = () => {
    if (!html || !generatedVideoUrl) {
          ...
          <AnimatePresence initial={false}>
            ...
            {!sidebarCollapsed && activePanel === 'templates' && (
              <TemplatesPanel
                isDark={isDark}
                quickStartTemplates={quickStartTemplates}
                onQuickStart={runQuickStartTemplate}
                supabaseTemplates={supabaseTemplates}
                loadingTemplates={loadingTemplates}
                onLoadSupabase={loadSupabaseTemplate}
                mobileBusy={isGenerating || isThinking}
                onStartMobile={startMobileTemplate}
              />
            )}
            ...
            {!sidebarCollapsed && activePanel === 'images' && (
              <ImagesPanel
                ...
                onDeselectMedia={() => { setSelectedMediaElement(null); setShowMediaReplacer(false) }}
                ...
                onClearGenerated={() => { setGeneratedImageUrl(null); addConsoleLog('info', 'Image removed') }}
                onInsert={insertImageIntoWebsite}
            ...
            {!sidebarCollapsed && activePanel === 'video' && (
              <VideoPanel
                ...
                videoSourceUploading={videoSourceUploading}
                onUploadSource={uploadVideoSource}
                onClearSource={() => setVideoSourceImage(null)}
            ...
            {!sidebarCollapsed && activePanel === 'deploy' && (
              <ShipPanel
                ...
                onPublishInstant={publishInstant}
                ...
                isSearchingDomain={isSearchingDomain}
                domainSearched={domainSearched}
                domainResults={domainResults}
                onBuyDomain={buyDomain}
                ownDomainInput={ownDomainInput}
                onOwnDomainInputChange={setOwnDomainInput}
                onConnectOwnedDomain={connectOwnedDomain}
                isConnectingDomain={isConnectingDomain}
                connectedDomain={connectedDomain}
                onOpenConsole={() => setActivePanel('console')}
                addToast={addToast}
              />
            )}
          </AnimatePresence>
