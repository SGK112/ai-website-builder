...
export async function POST(req: NextRequest) {
  ...
  return streamJsonWithHeartbeats(async () => {
    ...
    console.info(`[Astro Builder] Generated "${name}" — ${Object.keys(files).length} files, ${Math.round(rawText.length / 1024)}KB raw, ${attempts} attempt(s)`)

    try {
      const { recordCompletedBuild } = await import('@/lib/pending-builds')
      await recordCompletedBuild({
        userId: gate.userId, kind: 'astro', prompt,
