      ...
      <div className="relative w-full max-w-md max-h-[85dvh] overflow-y-auto rounded-2xl bg-zinc-950 border border-border shadow-2xl">
        ...
        {done ? (
          <div className="p-5 text-center">
            ...
            <div className="text-sm font-semibold text-foreground">Listing submitted</div>
            <p className="text-xs text-muted-foreground mt-1.5">It goes live in the community after a quick review. To actually receive payouts, finish your Stripe payout setup in your profile — buyers can't check out against a seller who hasn't.</p>
            <div className="flex gap-2 mt-4">
              <a href="/community" className="flex-1 flex items-center justify-center gap-1 rounded-lg bg-muted hover:bg-muted/70 text-foreground text-xs py-2">View community <ExternalLink className="w-3 h-3" /></a>
            ...
            <button onClick={onClose} className="mt-2 text-[11px] text-muted-foreground hover:text-foreground">Done</button>
          </div>
        ) : (
          <div className="p-4 space-y-3">
            ...
            <div>
              ...
              <input value={title} onChange={e => setTitle(e.target.value)} className="w-full bg-muted/50 border border-border rounded-lg px-2.5 py-1.5 text-sm text-foreground focus:outline-none focus:border-emerald-500/50" />
            </div>
            <div>
              <label className="block text-[11px] text-muted-foreground mb-1">Description <span className="text-muted-foreground">(optional)</span></label>
              <textarea value={description} onChange={e => setDescription(e.target.value)} rows={2} placeholder="What it's good for, how to use it…" className="w-full bg-muted/50 border border-border rounded-lg px-2.5 py-1.5 text-xs text-foreground placeholder:text-muted-foreground focus:outline-none focus:border-emerald-500/50 resize-none" />
            </div>
            <div>
              <label className="block text-[11px] text-muted-foreground mb-1">Price (USD)</label>
              <div className="flex items-center gap-2">
                <span className="text-muted-foreground text-sm">$</span>
                <input type="number" min={0} step={1} value={priceUsd} onChange={e => setPriceUsd(e.target.value)} className="w-24 bg-muted/50 border border-border rounded-lg px-2.5 py-1.5 text-sm text-foreground focus:outline-none focus:border-emerald-500/50" />
                <span className="text-[11px] text-muted-foreground">{priceCredits > 0 ? `You keep ~$${youKeep} (97%)` : 'Free'}</span>
              </div>
            </div>
            {error && <div className="text-xs text-red-400 bg-red-500/10 border border-red-500/20 rounded-lg px-3 py-2">{error}</div>}
            <p className="text-[10px] text-muted-foreground">Listings are reviewed before going live. Payouts require Stripe setup in your profile.</p>
