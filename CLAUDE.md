...
- **MongoDB (cluster shared with VoiceNow).** The live prod `MONGODB_URI`'s default DB is **`ai-website-builder`**, and webstew's Users/Projects/Credits/published_sites/app_data/community/`formsubmissions` all live there — verified 2026-06-26 by an end-to-end prod session sim (a user created in that DB authenticated + published on www.webstew.net). The Mongo cluster is shared with VoiceNow/Aria and the Stripe account is shared (see below), so treat user/project/billing data as cross-product-sensitive. ⚠️ Earlier docs claimed the default DB was `voiceflow-crm` with an `app:'webstew'` marker — that is **outdated**; webstew's live data is in `ai-website-builder`. (Mongoose model→collection gotcha: `FormSubmission` → `formsubmissions`, no underscore.)
